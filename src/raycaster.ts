...
import type { LevelDefinition, SpriteRenderable, Vec2 } from './types';
import { distance, vec } from './math';
...
export class Raycaster {
  ...
  setTextures(images: HTMLCanvasElement[]) {
  ...
  cast(origin: Vec2, direction: Vec2): RayHit {
    ...
    while (!hit) {
      ...
      if (tileIndex > 0) {
        hit = true;
        const perpWallDist = side === 0 ? (currentX - origin.x + (1 - stepX) / 2) / direction.x : (currentY - origin.y + (1 - stepY) / 2) / direction.y;
  ...
  gatherSprites(playerPos: Vec2, sprites: SpriteRenderable[]): SpriteRenderable[] {
